...
<main><div class="brand"><div class="mark">◖</div><div><h1>MQTT Call</h1><div class="sub">Linux listener · encrypted voice</div></div></div><section class="status"><b id="state">Disconnected</b><div id="speaker" class="detail"></div><div id="topic" class="detail">call/channel/3344</div></section>
...
async function disconnectNow(){await fetch('/disconnect',{method:'POST'});refresh()}async function refresh(){try{let s=await(await fetch('/status')).json();$("state").textContent=s.state;$("speaker").textContent=s.speaker?"Talking: "+s.speaker:"";$("topic").textContent=s.topic;$("buffer").textContent=s.buffer+" · "+s.queued+" queued";$("batches").textContent=s.batches+" packets";$("error").textContent=s.error||"";let a=!['Disconnected','Error','Audio error'].includes(s.state);$("connect").disabled=a;$("disconnect").disabled=!a}catch{}}setInterval(refresh,500);refresh();
</script></html>'''
